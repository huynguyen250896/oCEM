...
IPCAfdr=function(data = NULL, clinical = NULL, ncomp = NULL, standardize = T,
  ...
  IPCA = mixOmics::ipca(data, ncomp = ncomp, fun = "logcosh", mode="deflation")
  ...
  ind=data.frame(My_name_is=paste0("Huy", 1:length(kurtosis_res)), index = NA)
  for (j in 1:length(kurtosis_res)){
    if(kurtosis_res[j] < 3){
      ind$index[j] = j
    }
  ...
  names(fdr) = gsub("IPC", "module", names(fdr))
  ...
  names(fdr) = gsub('IPC', '', names(fdr), perl = T)
