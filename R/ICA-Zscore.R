...
ICAzscore=function(data = NULL, clinical=NULL, ncomp = NULL, standardize = T,
  ...
  scaled_exp = scale(t(kurt)) %>% #do Z-score transformation on components (columns)
    t() %>%
    data.frame()
  names(scaled_exp) = gsub("X", "module", names(scaled_exp))
  cat(">> Please choose a standard deviation threshold to let oCEM be able to assign genes to modules specifically.", "\n")
  ...
  scaled_exp <- apply(scaled_exp, 1, function(x) ifelse(abs(x) > threshold, 1, 0)) %>% t()

  #signatures whose all elements in the columns equal 0
  remove_comp <- which(colSums(scaled_exp) == 0) 

  #In case the user chooses a too big threshold, all signatures will have their all entries in every column that equal 0
  test_scaled_exp <- scaled_exp[,-remove_comp]
  if( (length(remove_comp) > 0) && (ncol(test_scaled_exp) == 0) ){
    stop("Threshold of your choice is too big, please choose another smaller one again!")
  }

  #empty signatures will be removed
  if( (length(remove_comp) > 0) ){ #have colSums = 0
    scaled_exp <- scaled_exp %>% dplyr::select(-all_of(names(remove_comp))) # remove columns whose all elements equal 0
  ...
  if( any(rowSums(scaled_exp) == 0) ){
    ind3 <- which(rowSums(scaled_exp) == 0)
    grey_module <- rep(0, nrow(scaled_exp))
    for (i in ind3){
      grey_module[i] <- grey_module[[i]] + 1
    }
    scaled_exp = data.frame(scaled_exp) %>% dplyr::mutate("0" = grey_module)
  }
  colnames(scaled_exp) <- gsub("module", "", colnames(scaled_exp))

  #convert numeric lables into colors
  names(scaled_exp) = gsub('X', '', names(scaled_exp), perl = T)
  moduleColors <- WGCNA::labels2colors(as.numeric(colnames(scaled_exp)))
  colnames(scaled_exp) <-  moduleColors

  #determine the exact number of genes distributed to a certain module
  component_summary <- colSums(scaled_exp)
  ...
  if( !is.null(clinical) ){
    ...
    if( "grey" %in% colnames(scaled_exp) ){
  ...
  if( "grey" %in% colnames(scaled_exp) ){
    scaled_exp_without_grey <- scaled_exp[,( 1:(ncol(scaled_exp) -1) )]
    kurt1 <- kurt1 * scaled_exp_without_grey %>% as.data.frame() %>% mutate_all(., function(x) as.numeric(as.character(x)))
    kurt1[kurt1 == 0] <- NA_integer_
  } else{
    kurt1 <- kurt1 * scaled_exp %>% as.data.frame() %>% mutate_all(., function(x) as.numeric(as.character(x)))
    kurt1[kurt1 == 0] <- NA_integer_
  }
  ...
  res <- list(modules = as.data.frame(scaled_exp),
